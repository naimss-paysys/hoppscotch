...
| Primary IP        | 10.0.150.126                                             |
| All IPs on host   | 10.0.150.126, 172.18.0.1, 172.21.0.1, 172.17.0.1,      |
...
| Keycloak port     | 8080                                                     |
| User              | abdul.hafeez (sudo access)                               |

## /etc/hosts Entry (every dev machine needs this)
```
10.0.150.126  api.paysyslabs.com
```
...
- They communicate via HOST IP: 10.0.150.126 (NOT shared Docker network)
...
groups | grep docker
# If not: sudo usermod -aG docker abdul.hafeez && newgrp docker
```
...
| Client secret       | hoppscotchsecret123 |
| DB user             | keycloak            |
...
|---------------------|-----------------------------------|
| DB user             | hoppscotch                        |
| DB password         | hoppscotch123                     |
| DB name             | hoppscotch                        |
| Redis password      | redis123                          |
| JWT secret          | paysyslabsjwtsecretkey32charslong |
| Refresh secret      | paysyslabsrefreshtokensecret32ch  |
| Session secret      | paysyslabssessionsecret32charss   |
...
# Test API health
curl http://10.0.150.126:8090/api/ping
